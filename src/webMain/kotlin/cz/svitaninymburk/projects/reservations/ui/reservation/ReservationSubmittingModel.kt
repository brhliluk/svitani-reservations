...
abstract class ReservationSubmittingModel(
    ...
    fun submitReservation(target: ReservationTarget, formData: ReservationFormData, userId: Uuid?) {
        scope.launch {
            isSubmitting = true
            try {
                submitter.submit(target, formData, userId)
                    .onRight { onReserved(it) }
                    .onLeft { showToast(reservationErrorMessage(it.localizedMessage(currentStrings)), ToastType.Error) }
