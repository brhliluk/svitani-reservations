...
class FreeReservationsBackfillTest {
    ...
    fun `pending reservation with zero price becomes confirmed and free`() {
        ...
        transaction(db) {
            SchemaUtils.create(ReservationsTable)
            insertReservation(id, totalPrice = 0.0, status = Reservation.Status.PENDING_PAYMENT, paymentType = PaymentInfo.Type.BANK_TRANSFER)
        }
        ...
        transaction(db) {
            ...
            assertEquals(PaymentInfo.Type.FREE, row[ReservationsTable.paymentType])
    ...
    fun `pending reservation with a price is left untouched`() {
        ...
        transaction(db) {
            SchemaUtils.create(ReservationsTable)
            insertReservation(id, totalPrice = 150.0, status = Reservation.Status.PENDING_PAYMENT, paymentType = PaymentInfo.Type.BANK_TRANSFER)
        }
        ...
        transaction(db) {
            ...
            assertEquals(PaymentInfo.Type.BANK_TRANSFER, row[ReservationsTable.paymentType])
        }
    }
    ...
    fun `waitlisted zero price reservation stays in the waitlist`() {
        ...
        transaction(db) {
            SchemaUtils.create(ReservationsTable)
            insertReservation(id, totalPrice = 0.0, status = Reservation.Status.WAITLISTED, paymentType = PaymentInfo.Type.FREE)
        }
    ...
    fun `cancelled zero price reservation stays cancelled`() {
        ...
        transaction(db) {
            SchemaUtils.create(ReservationsTable)
            insertReservation(id, totalPrice = 0.0, status = Reservation.Status.CANCELLED, paymentType = PaymentInfo.Type.FREE)
        }
